# FaceCue ML - Integrated Nutritional Deficiency Detection
# Combines lifestyle data and facial analysis for enhanced nutritional deficiency detection
...
class IntegratedNutritionalDeficiencyDetector:
    ...
    def _initialize_deficiency_recommendations(self):
        ...
        return {
            'Normal': {
                ...
                'description': 'Your nutritional status appears to be within normal ranges based on lifestyle and facial analysis.',
                'recommendations': [
            ...
            'Iron Deficiency (Anemia)': {
                ...
                'description': 'Your lifestyle patterns and facial indicators suggest iron deficiency anemia requiring immediate attention.',
            ...
            'Vitamin D Deficiency': {
                ...
                'description': 'Your lifestyle and facial indicators suggest vitamin D deficiency requiring assessment.',
            ...
            'Vitamin B12 Deficiency': {
                ...
                'description': 'Your lifestyle and facial indicators suggest vitamin B12 deficiency requiring immediate evaluation.',
            ...
            'Vitamin C Deficiency': {
                ...
                'description': 'Your lifestyle and facial indicators suggest potential vitamin C deficiency.',
            ...
            'Zinc Deficiency': {
                ...
                'description': 'Your lifestyle and facial indicators suggest zinc deficiency affecting immune function.',
            ...
            'Magnesium Deficiency': {
                ...
                'description': 'Your lifestyle and facial indicators suggest magnesium deficiency affecting muscle and nerve function.',
            ...
            'Multiple Deficiencies': {
                ...
                'description': 'Your lifestyle and facial indicators suggest multiple nutritional deficiencies requiring comprehensive evaluation.',
    ...
    def create_integrated_dataset(self):
        """Create dataset that combines lifestyle and facial features"""
        print("=== Creating Integrated Nutritional Deficiency Dataset ===")
        ...
        # Generate facial features that correlate with nutritional deficiencies
        facial_features = {
            'skin_pallor_score': np.random.normal(0.5, 0.2, n_samples),
            'eye_brightness': np.random.normal(0.6, 0.2, n_samples),
            'lip_color_vitality': np.random.normal(0.5, 0.2, n_samples),
            'face_symmetry': np.random.normal(0.7, 0.15, n_samples),
            'skin_health_score': np.random.normal(0.6, 0.2, n_samples),
            'fatigue_indicators': np.random.normal(0.4, 0.2, n_samples)
        }
        
        # Add facial features to dataset
        for feature, values in facial_features.items():
            df[feature] = values
        
        # Create nutritional deficiency labels based on BOTH lifestyle AND facial patterns
        deficiency_scores = []
        for i in range(n_samples):
            score = 0
            
            # Iron deficiency indicators (lifestyle + facial)
            if df.iloc[i]['diet_type'] == 'Vegan' and df.iloc[i]['fatigue_scale'] > 3:
                score += 2
            elif df.iloc[i]['fatigue_scale'] > 4 and df.iloc[i]['energy_scale'] < 2:
                score += 1
            
            # Add facial pallor indicators for iron deficiency
            if df.iloc[i]['skin_pallor_score'] > 0.7:  # High pallor
                score += 1
            if df.iloc[i]['lip_color_vitality'] < 0.3:  # Low lip vitality
                score += 1
            
            # Vitamin D deficiency indicators (lifestyle + facial)
            if df.iloc[i]['sun_exposure'] == 'Low' and df.iloc[i]['diet_type'] == 'Vegan':
                score += 2
            elif df.iloc[i]['sun_exposure'] == 'Low':
                score += 1
            
            # Add facial indicators for vitamin D deficiency
            if df.iloc[i]['skin_health_score'] < 0.4:  # Poor skin health
                score += 1
            
            # Vitamin B12 deficiency indicators (lifestyle + facial)
            ...
            # Add facial indicators for B12 deficiency
            if df.iloc[i]['fatigue_indicators'] > 0.6:  # High fatigue indicators
                score += 1
            
            # Vitamin C deficiency indicators (lifestyle + facial)
            if df.iloc[i]['fruit_vegetable_intake'] == 'Low' and df.iloc[i]['fatigue_scale'] > 3:
                score += 1
            
            # Add facial indicators for vitamin C deficiency
            if df.iloc[i]['skin_health_score'] < 0.5:  # Poor skin condition
                score += 1
            
            # Zinc deficiency indicators (lifestyle + facial)
            if df.iloc[i]['diet_type'] == 'Vegan' and df.iloc[i]['meal_frequency'] == '1-2 meals':
                score += 1
            
            # Add facial indicators for zinc deficiency
            if df.iloc[i]['skin_health_score'] < 0.4:  # Poor skin health
                score += 1
            
            # Magnesium deficiency indicators (lifestyle + facial)
            if df.iloc[i]['stress_level'] > 4 and df.iloc[i]['exercise_hours'] < 2:
                score += 1
            
            # Add facial indicators for magnesium deficiency
            if df.iloc[i]['face_symmetry'] < 0.6:  # Poor face symmetry
                score += 1
        ...
        print(f"✓ Integrated dataset created: {df.shape}")
        print(f"✓ Deficiency distribution: {df['deficiency_type'].value_counts().to_dict()}")
        
        return df
    
    def preprocess_integrated_data(self, df):
        """Preprocess integrated lifestyle and facial data"""
        print("\n=== Preprocessing Integrated Data ===")
        ...
        # Prepare features (lifestyle + facial)
        feature_cols = [col for col in df_processed.columns if col not in ['deficiency_type', 'target']]
        self.feature_names = feature_cols
        
        print(f"✓ Features prepared: {len(feature_cols)} (lifestyle + facial)")
        print(f"✓ Deficiency types: {len(df_processed['target'].unique())}")
        
        return df_processed
    
    def train_integrated_model(self, df):
        """Train integrated model for nutritional deficiency detection"""
        print("\n=== Training Integrated Nutritional Deficiency Model ===")
        ...
        self.model = RandomForestClassifier(n_estimators=300, random_state=42, max_depth=20)
        ...
        print(f"✓ Integrated nutritional deficiency model trained")
        ...
        print(f"\nTop 15 Most Important Features (Lifestyle + Facial):")
        print(feature_importance.head(15))
        
        return accuracy
    
    def predict_integrated_deficiency(self, lifestyle_features, facial_features=None):
        """Predict nutritional deficiency from integrated lifestyle and facial data"""
        if self.model is None:
            return None, None
        
        try:
            # Combine lifestyle and facial features
            combined_features = lifestyle_features.copy()
            
            if facial_features:
                # Add facial features to lifestyle features
                combined_features.update(facial_features)
            else:
                # Use default facial features if not provided
                default_facial = {
                    'skin_pallor_score': 0.5,
                    'eye_brightness': 0.6,
                    'lip_color_vitality': 0.5,
                    'face_symmetry': 0.7,
                    'skin_health_score': 0.6,
                    'fatigue_indicators': 0.4
                }
                combined_features.update(default_facial)
            ...
            for col in ['gender', 'diet_type', 'smoking', 'alcohol_consumption', 
                ...
                if col in combined_features:
                    ...
                    feature_df[f'{col}_encoded'] = le.transform([combined_features[col]])[0]
                    del feature_df[col]
    ...
    def generate_integrated_recommendations(self, deficiency, confidence=None, facial_analysis=None):
        ...
        # Add facial analysis insights if available
        if facial_analysis and facial_analysis.get('success'):
            facial_insights = facial_analysis['analysis']['insights']
            facial_recs = facial_analysis['analysis']['recommendations']
            
            if facial_insights:
                rec['facial_insights'] = facial_insights
            if facial_recs:
                rec['facial_recommendations'] = facial_recs
        
        return rec
    
    def create_integrated_assessment_report(self, lifestyle_features, facial_features, user_name="User"):
        """Create comprehensive integrated nutritional deficiency assessment report"""
        print(f"\n=== Integrated Nutritional Deficiency Assessment for {user_name} ===")
        
        # Make integrated prediction
        deficiency, probabilities = self.predict_integrated_deficiency(lifestyle_features, facial_features)
        
        if deficiency is None:
            return "Unable to generate integrated assessment"
        
        # Calculate confidence
        confidence = max(probabilities) if probabilities is not None else None
        
        # Generate integrated recommendations
        recommendations = self.generate_integrated_recommendations(deficiency, confidence, facial_features)
        ...
        # Print report
        print(f"\n🍎 INTEGRATED NUTRITIONAL DEFICIENCY ASSESSMENT")
        ...
        # Show integration of both data sources
        print(f"\n🔗 DATA INTEGRATION:")
        print(f"  • Lifestyle data: {len(lifestyle_features)} features")
        if facial_features:
            print(f"  • Facial analysis: {len(facial_features)} features")
            print(f"  • Combined analysis: Enhanced accuracy")
        else:
            print(f"  • Facial analysis: Not available")
            print(f"  • Analysis: Lifestyle data only")
        ...
        # Add facial-based recommendations if available
        if 'facial_recommendations' in recommendations:
            print(f"\n📸 FACIAL-BASED ADDITIONAL RECOMMENDATIONS:")
            for i, rec in enumerate(recommendations['facial_recommendations'], len(recommendations['recommendations']) + 1):
                print(f"  {i:2d}. {rec}")
        ...
        print("  This integrated assessment combines lifestyle and facial analysis.")
    ...
    def save_integrated_model(self, filename='data/integrated_nutritional_model.pkl'):
        """Save the integrated nutritional deficiency model"""
        model_data = {
            'model': self.model,
            'scaler': self.scaler,
        ...
        print(f"✓ Integrated nutritional deficiency model saved: {filename}")
        return filename
    
    def load_integrated_model(self, filename='data/integrated_nutritional_model.pkl'):
        """Load the integrated nutritional deficiency model"""
        try:
            ...
            print(f"✓ Integrated nutritional deficiency model loaded: {filename}")
            return True
        except FileNotFoundError:
            print(f"❌ Integrated nutritional deficiency model not found: {filename}")
            return False
        except Exception as e:
            print(f"❌ Error loading integrated nutritional deficiency model: {e}")
            return False

def main():
    print("🍎 FaceCue ML - Integrated Nutritional Deficiency Detection System")
    print("="*70)
    
    # Initialize integrated detector
    detector = IntegratedNutritionalDeficiencyDetector()
    
    # Create and preprocess integrated data
    df = detector.create_integrated_dataset()
    df_processed = detector.preprocess_integrated_data(df)
    
    # Train integrated model
    accuracy = detector.train_integrated_model(df_processed)
    
    # Save integrated model
    detector.save_integrated_model()
    
    # Example integrated assessments
    print(f"\n=== Example Integrated Nutritional Deficiency Assessments ===")
    
    # Vegan with potential deficiencies (lifestyle + facial)
    vegan_lifestyle = {
        'age': 25,
        'gender': 'Female',
        'diet_type': 'Vegan',
    ...
    vegan_facial = {
        'skin_pallor_score': 0.8,  # High pallor
        'eye_brightness': 0.3,      # Low brightness
        'lip_color_vitality': 0.2,  # Low vitality
        'face_symmetry': 0.6,      # Moderate symmetry
        'skin_health_score': 0.3,   # Poor skin health
        'fatigue_indicators': 0.8   # High fatigue
    }
    
    detector.create_integrated_assessment_report(vegan_lifestyle, vegan_facial, "Vegan Student with Facial Analysis")
    
    print(f"\n=== Integrated Nutritional Deficiency Detection System Complete ===")
    print(f"✅ Model trained with {accuracy:.3f} accuracy")
    print(f"✅ 8 specific nutritional deficiencies detected")
    print(f"✅ Integrated lifestyle + facial analysis")
